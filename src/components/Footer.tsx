...
const Footer = () => {
  ...
  if (loading || !content) {
    return <footer className="bg-midnight-blue text-silver">
  ...
  return <>
      <footer className="bg-black text-white">
        <div className="container mx-auto container-padding py-20 flex flex-col items-center justify-center text-center">
          ...
          <div className="mb-8">
            <ul className="flex flex-wrap justify-center gap-8">
              {footerContent.contact.items.map(item => <li key={item.label}>
                  {item.action === 'demo' ? <button onClick={handleRequestDemo} className="text-lg font-semibold hover:text-accent transition-colors">
                      {item.label}
                    </button> : item.action === 'scroll' && item.target ? <button onClick={() => scrollToSection(item.target)} className="text-lg font-semibold hover:text-accent transition-colors">
                      {item.label}
                    </button> : <span className="text-lg font-semibold">{item.label}</span>}
                </li>)}
            </ul>
          </div>
          ...
          <div className="border-t border-white w-full pt-8 mt-8">
            ...
            <div className="flex flex-wrap justify-center gap-8">
              {footerContent.legal.links.map(link => <Link key={link.label} to={link.path} className="text-base font-medium hover:text-accent transition-colors">
                  {link.label}
                </Link>)}
            </div>
          </div>
